...
//#define PMDEBUG //tag to turn on debugging for PM code
#ifdef PMDEBUG
#include <assert.h>
#endif
...
static int *nwin_list,*wlen_list,*wmin_list, *wmax_list;
...
void pm_init_periodic(void)
{
  ...
  nwin_list = mymalloc(NTask * sizeof(int));
  wlen_list = mymalloc(NTask * sizeof(int));
  wmin_list = mymalloc(NTask * sizeof(int));
  wmax_list = mymalloc(NTask * sizeof(int));
...
void pmforce_periodic(void)
{
  ...
  /* now setup window width for more memory efficient PM algorithm */
  int wlen,Nwin,Nwinmax,wmin,wmax,wind,Nslab,Nw;
  double tb0,tb1,twbar = 0.0,tlbar = 0.0,tpd = 0.0,tl = 0.0,tl0,tl1;
  double tmin,tmax,tsm = 0.0,tsm0,tsm1;
  
  dimprod = (dimy + 4) * (dimz + 4);
  //MPI_Allreduce(&dimprod, &dimprodmax, 1, MPI_INT, MPI_MAX, MPI_COMM_WORLD);
  dimprodmax = dimprod;
  wlen = PM_MEM_FAC*maxfftsize/dimprodmax - 5;
  if(wlen <= 0)
    wlen = 1;
  MPI_Allgather(&wlen,1,MPI_INT,wlen_list,1,MPI_INT,MPI_COMM_WORLD);
  
  MPI_Allreduce(&wlen,&wmin,1,MPI_INT,MPI_MIN,MPI_COMM_WORLD); 
  MPI_Allreduce(&wlen,&wmax,1,MPI_INT,MPI_MAX,MPI_COMM_WORLD); 
  if(ThisTask == 0)
    {
      printf("window length min,max = %d|%d\n",wmin,wmax);
      fflush(stdout);
    }
  
  /* now allocate memory to hold the FFT fields */
  
  dimprod = (wlen + 5) * (dimy + 4) * (dimz + 4);
  MPI_Allreduce(&dimprod, &dimprodmax, 1, MPI_INT, MPI_MAX, MPI_COMM_WORLD);
  ...
  if(!(forcegrid = (fftw_real *) mymalloc(bytes = imax(maxfftsize, dimprodmax) * sizeof(fftw_real))))
    {
  ...
  MPI_Reduce(&mbyte, &mbytesum, 1, MPI_DOUBLE, MPI_SUM, drsmax.r, MPI_COMM_WORLD);
  
  if(ThisTask == drsmax.r)
    {
      fprintf(stdout,"allocated memory for PM step: %g|%g|%g MB for rho,force,work, max dim of local patch = %d|%d|%d.\n",
              ...
              dimx,dimy,dimz);
      printf("Use of %g MByte on average for PM calculation.\n", mbytesum / NTask);
      fflush(stdout);
    }
  
#ifdef PMDEBUG
  int *usep;
  if(!(usep = (int*)mymalloc(sizeof(int)*NumPart)))
    {
      printf("failed to allocate memory for PM DEBUG.\n");
      endrun(1);
    }
  for(i=0;i<NumPart;++i)
    usep[i] = 0;
  ...
  //share density field
  // loop through PM grid in x direction with window of length wlen
  // particle contributions to each wlen set of slabs are shared
  t0 = second();
  
  Nwinmax = -1;
  for(i=0;i<NTask;++i)
    {
      Nslab = meshmax_list[3*i + 0] - meshmin_list[3*i + 0] + 2;
      Nwin = Nslab/wlen_list[i];
      if(Nwin*wlen_list[i] < Nslab)
	++Nwin;
      nwin_list[i] = Nwin;
      if(Nwin > Nwinmax)
	Nwinmax = Nwin;
    }
  Nslab = meshmax[0] - meshmin[0] + 2;
  Nwin = nwin_list[ThisTask];
  //MPI_Allreduce(&Nwin,&Nwinmax,1,MPI_INT,MPI_MAX,MPI_COMM_WORLD);
  
  if(ThisTask == 0)
    {
      printf("start sharing density field w/ %d windows\n",Nwinmax);
      fflush(stdout);
    }
  ...
  for(wind=0;wind<Nwinmax;++wind)
    {
      tb0 = second();
      //some processors will have more windows than others
      for(i=0;i<NTask;++i)
	{
	  if(wind < nwin_list[i])
	    {
	      //wmin and wmax have the first and last index of local PM cells
	      wmin_list[i] = wind*wlen_list[i] + meshmin_list[3*i + 0];
	      wmax_list[i] = wmin_list[i] + wlen_list[i] - 1;
	      if(wmax_list[i] > (meshmax_list[3*i + 0] - meshmin_list[3*i + 0] + 2) + meshmin_list[3*i+0] - 1)
		wmax_list[i] = (meshmax_list[3*i + 0] - meshmin_list[3*i + 0] + 2) + meshmin_list[3*i+0] - 1;
	    }
	  else
	    {
	      wmin_list[i] = 0;
      ...
      wmin = wmin_list[ThisTask];
      wmax = wmax_list[ThisTask];
      Nw = wmax - wmin + 1;
      
      //MPI_Allgather(&wmin,1,MPI_INT,wmin_list,1,MPI_INT,MPI_COMM_WORLD);
      //MPI_Allgather(&wmax,1,MPI_INT,wmax_list,1,MPI_INT,MPI_COMM_WORLD);
      
      for(i = 0; i < Nw * dimy * dimz; i++)
	workspace[i] = 0;
      ...
      if(Nw > 0)
	{
	  for(i = 0; i < NumPart; i++)
		...
		slab_x = PMGRID - 1;
	      ...
	      //skip particles not in current window
	      if((slab_x >= 0 && slab_x < Nw) || (slab_xx >= 0 && slab_xx < Nw))
		{
      ...
      tb1 = second();
      tpd += timediff(tb0,tb1);
      
#ifdef PMDEBUG
      if(ThisTask == 0)
	{
	  sprintf(pmdbfname,"%spmdebug_step%d_window%d.dat",All.OutputDir, All.NumCurrentTiStep, wind);
	  pmdbfp = fopen(pmdbfname,"w");
	  fprintf(pmdbfp,"# task mmin mmax wmin wmax pmin pmax\n");
	  for(i=0;i<NTask;++i)
	    fprintf(pmdbfp,"%d %d %d %d %d %d %d\n",i,meshmin_list[3*i],meshmax_list[3*i],
		    wmin_list[i],wmax_list[i],
		    first_slab_of_task[i],first_slab_of_task[i]+slabs_per_task[i]-1);
	  fclose(pmdbfp);
	}
      sprintf(pmdbfname,"%spmdebug_step%d_window%d.%d",All.OutputDir, All.NumCurrentTiStep, wind, ThisTask);
      pmdbfp = fopen(pmdbfname,"w");
      fprintf(pmdbfp,"# stask smin smax rtask\n");
#endif
      ...
      for(level = 0; level < (1 << PTask); level++)	/* note: for level=0, target is the same task */
	{
	  ...
	  if(recvTask < NTask)
	    {
	      ...
	      if(wmax >= 0)
		{
		  for(slab_x = wmin; slab_x <= wmax; slab_x++)
		...
		sendmin = 0;
	      ...
	      recvmin = 2 * PMGRID;
	      recvmax = -1;
	      if(wmax_list[recvTask] >= 0)
		{
		  for(slab_x = wmin_list[recvTask]; slab_x <= wmax_list[recvTask]; slab_x++)
		...
		{
		  recv_dimx = wmax_list[recvTask] - wmin_list[recvTask] + 1;
		  ...
		  if((sendmax - sendmin) >= 0)
		    fprintf(pmdbfp,"%d %d %d %d\n",ThisTask,sendmin,sendmax,recvTask);
#endif
		  ...
		  if(level > 0)
		    {
		      tsm0 = second();
		      MPI_Sendrecv(workspace + (sendmin - wmin) * dimy * dimz,
		      ...
		      tsm1 = second();
		      tsm += timediff(tsm0,tsm1);
		    }
		  else
		    {
		      memcpy(forcegrid, workspace + (sendmin - wmin) * dimy * dimz,
      ...
      //MPI_Barrier(MPI_COMM_WORLD);
      //tb1 = second();
      //twbar += timediff(tb0, tb1);
    } //window loop
  
  //MPI_Allreduce(&twbar,&tmin,1,MPI_DOUBLE,MPI_MIN,MPI_COMM_WORLD);
  //MPI_Allreduce(&twbar,&tmax,1,MPI_DOUBLE,MPI_MAX,MPI_COMM_WORLD);
  //if(ThisTask == 0)
  //{
  //  printf("barrier time min,max = %g|%g\n",tmin,tmax);
  //  fflush(stdout);
  //}
  
  MPI_Allreduce(&tsm,&tmin,1,MPI_DOUBLE,MPI_MIN,MPI_COMM_WORLD);
  MPI_Allreduce(&tsm,&tmax,1,MPI_DOUBLE,MPI_MAX,MPI_COMM_WORLD);
  if(ThisTask == 0)
    {
      printf("sendrecv time min,max = %g|%g\n",tmin,tmax);
      fflush(stdout);
    }
  ...
  if(tsm == tmax)
    {
      fprintf(stdout,"%03d: tpd,tl = %6g|%6g, tsm,tlbar,twbar = %6g|%6g|%6g, wlen = %3d, Nwin = %3d, mesh length = %4d|%4d|%4d\n",
	      ThisTask,tpd,tl,tsm,tlbar,twbar,wlen,Nwin,
	      meshmax[0]-meshmin[0]+1,meshmax[1]-meshmin[1]+1,meshmax[2]-meshmin[2]+1);
      fflush(stdout);
    }
  ...
  for(i=0;i<NTask;++i)
    {
      Nslab = meshmax_list[3*i + 0] - meshmin_list[3*i + 0] + 1;
      Nwin = Nslab/wlen_list[i];
      if(Nwin*wlen_list[i] < Nslab)
        ++Nwin;
      nwin_list[i] = Nwin;
  ...
  Nslab = meshmax[0] - meshmin[0] + 1;
  Nwin = nwin_list[ThisTask];
  //MPI_Allreduce(&Nwin,&Nwinmax,1,MPI_INT,MPI_MAX,MPI_COMM_WORLD);
  
  if(ThisTask == 0)
    {
      printf("start assembling local potential w/ %d windows\n",Nwinmax);
      fflush(stdout);
    }
  ...
  for(wind=0;wind<Nwinmax;++wind)
    {
          ...
          if(wind < nwin_list[i])
            {
              ...
              wmin_list[i] = wind*wlen_list[i] + meshmin_list[3*i + 0];
              wmax_list[i] = wmin_list[i] + wlen_list[i] - 1;
              if(wmax_list[i] > (meshmax_list[3*i + 0] - meshmin_list[3*i + 0] + 1) + meshmin_list[3*i+0] - 1)
                wmax_list[i] = (meshmax_list[3*i + 0] - meshmin_list[3*i + 0] + 1) + meshmin_list[3*i+0] - 1;
            }
          else
            {
              wmin_list[i] = 0;
      ...
      if(ThisTask == 0)
	{
	  sprintf(pmdbfname,"%spmdebug_force_step%d_window%d.dat",All.OutputDir, All.NumCurrentTiStep, wind);
	  pmdbfp = fopen(pmdbfname,"w");
	  fprintf(pmdbfp,"# task mmin mmax wmin wmax pmin pmax\n");
	  for(i=0;i<NTask;++i)
	    fprintf(pmdbfp,"%d %d %d %d %d %d %d\n",i,meshmin_list[3*i],meshmax_list[3*i],
      ...
      sprintf(pmdbfname,"%spmdebug_force_step%d_window%d.%d",All.OutputDir, All.NumCurrentTiStep, wind, ThisTask);
      pmdbfp = fopen(pmdbfname,"w");
      fprintf(pmdbfp,"# stask smin smax rtask\n");
#endif
	  ...
	  if(recvTask < NTask)
	    {
	      ...
	      if(wmax_list[recvTask] >= 0)
		{
		  for(slab_x = wmin_list[recvTask] - 2; slab_x < wmax_list[recvTask] + 4; slab_x++)
		...
		sendmin = sendmax + 1;
	      ...
	      if(wmax >= 0)
		{
		  for(slab_x = wmin - 2; slab_x < wmax + 4; slab_x++)
		...
		recvmin = recvmax + 1;
	      ...
	      if((recvmax - recvmin) >= 0 || (sendmax - sendmin) >= 0)	/* ok, we have a contribution to the slab */
		{
		  recv_dimx = wmax_list[recvTask] - wmin_list[recvTask] + 6;
		  ...
		  for(rep = 0; rep < ncont; rep++)
		      ...
		      if(level > 0)
			{
			  ...
			  MPI_Sendrecv(forcegrid,
				       ...
				       workspace + (recvmin - (wmin - 2)) * dimy * dimz,
			...
			{
			  memcpy(workspace + (recvmin - (wmin - 2)) * dimy * dimz,
...
void powerspec_save(void)
{
  ...
  if(ThisTask == 0)
    {
      sprintf(buf, "%slogs/powerspec_%03d.txt", All.OutputDir, All.PowerSpecFlag - 1);
