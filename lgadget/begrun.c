...
/* to be called by only one task */
void make_output_dirs(void) 
{
  char cmd[2048];

  printf("\n");
  printf("making output dir '%slogs'\n", All.OutputDir);
  printf("making output dir '%srestart'\n", All.OutputDir);
  fflush(stdout);
  
  sprintf(cmd,"mkdir -p %slogs", All.OutputDir);
  system(cmd);

  sprintf(cmd,"mkdir -p %srestart", All.OutputDir);
  system(cmd);
};
...
void open_outputfiles(void)
{
  ...
  sprintf(buf, "%slogs/%s", All.OutputDir, All.CpuFile);
  ...
  sprintf(buf, "%slogs/%s", All.OutputDir, All.InfoFile);
  ...
  sprintf(buf, "%slogs/%s", All.OutputDir, All.EnergyFile);
  ...
  sprintf(buf, "%slogs/%s", All.OutputDir, All.TimingsFile);
...
#ifdef DARKENERGY
  sprintf(buf, "%slogs/%s", All.OutputDir, "darkenergy.txt");
...
#ifdef LIGHTCONE
  sprintf(buf, "%slogs/%s", All.OutputDir, "lightcone.txt");
...
#define MAXTAGS 300
...
#ifdef ROCKSTAR
      strcpy(tag[nt], "RockstarExe");
      addr[nt] = All.RockstarExe;
      id[nt++] = STRING;

      strcpy(tag[nt], "RockstarConfig");
      addr[nt] = All.RockstarConfig;
      id[nt++] = STRING;
#endif
      ...
      if((fd = fopen(fname, "r")))
	{
	  ...
	  if(!(fdout = fopen(buf, "w")))
		...
		if(All.OutputDir[i - 1] != '/')
	      ...
	      sprintf(buf2, "%slogs/%s", All.OutputDir, "parameters-usedvalues");
  ...
  // prob not needed, since the bcast might have a barrier in it, but
  //  code make dirs above so adding the barrier here
